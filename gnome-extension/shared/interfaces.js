...
export const DEFAULT_ROOT_PATH = '/sys/class/net';
...
export function readIntegerFile(path, fallback = null) {
...
export class InterfaceCatalog {
    ...
    list() {
        ...
        try {
            ...
            while ((info = enumerator.next_file(null)) !== null) {
                ...
                const typeCode = readIntegerFile(`${this._rootPath}/${name}/type`, -1);
                const operState = readTextFile(`${this._rootPath}/${name}/operstate`) || 'unknown';
            ...
            console.error('Bandwidth Monitor GNOME: unable to enumerate interfaces.', error);
        } finally {
            if (enumerator)
    ...
    resolveSelection(selectionMode = 'auto', preferredInterface = '', includeTunnelInterfaces = false) {
        ...
        const selectionNote = selectionMode === 'preferred' && requestedInterface && selected
            ? `Preferred interface ${requestedInterface} is unavailable. Showing ${selected.name} instead.`
            : null;
