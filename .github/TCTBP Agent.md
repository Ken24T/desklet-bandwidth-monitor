...
This agent governs **milestone, checkpointing, publishing, handover, resume, sync, and deployment actions** for the Desklet Bandwidth Monitor repository. It exists to safely execute the agreed **TCTBP / SHIP workflow** with strong guard rails, auditability, and human approval at irreversible steps.
...
This repository uses `metadata.json` as the version source, plain semver tags such as `1.5.0`, `./scripts/validate-desklet.sh` as the normal verification gate, and the desklet package/install scripts for explicit deploy work.
...
For this repository, `metadata.json`, `settings-schema.json`, `*.js`, `*.py`, `*.css`, and `scripts/**` are not treated as docs-only by default.
...
2. **Assess whether SHIP is needed on the current branch**
   - If the current branch is non-default and has uncommitted changes or commits since the last `X.Y.Z` tag, recommend SHIP.
...
- If format is `keep-a-changelog`, move items from `[Unreleased]` to a new `[X.Y.Z]` heading.
...
`.github/TCTBP.json` is the repo's machine-readable reference.
